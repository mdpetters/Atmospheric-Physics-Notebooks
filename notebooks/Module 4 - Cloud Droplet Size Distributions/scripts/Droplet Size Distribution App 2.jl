...
xlabel = log10.([1e-6,1e-5,1e-4])
x = log10.(collect([1e-6:1e-6:9e-6;1e-5:1e-5:9e-5;1e-4]))
colors = "black", "darkred", "steelblue3"
df = [df1;df3;df2]

plot(layer(x = df[!,:D].*1e-6,y=df[!,:N],color=df[!,:Regime], Geom.line),
     ...
     Coord.cartesian(xmin=log10(1e-6), xmax=log10(1e-4)))
